"""Independent Code 39 regression: PNG, rasterized SVG and SVG embedded in PDF."""
...
# Стандартный набор читается как Code39Std, Full ASCII — как Code39Ext:
# в расширенном режиме пары вроде "+A" раскрываются в строчные буквы.
FORMATS = {
    "standard": zxingcpp.BarcodeFormat.Code39Std,
    "full-ascii": zxingcpp.BarcodeFormat.Code39Ext,
}


class Code39DecodeTest(unittest.TestCase):
    ...
    def test_all_cases_are_present(self):
        ...
        self.assertEqual(10, len({case["name"] for case in CASES}))
        self.assertEqual("digits", CASES[0]["name"])

    def assert_decodes(self, image, case):
        """Require ZXing-C++ to read the barcode as the expected text."""
        results = zxingcpp.read_barcodes(
            image.convert("RGB"), formats=FORMATS[case["charset"]], return_errors=True
        )
