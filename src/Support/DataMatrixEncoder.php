...
final class DataMatrixEncoder
{
    ...
    /**
     * Кодовое слово FNC1 в первой позиции: делает символ GS1 DataMatrix (идентификатор символики ]d2).
     */
     ...
     */
    public function encode(string $data, bool $gs1 = false): array
    {
        $encoded   = $this->codewords($data, $gs1);
    ...
    /**
     * Кодовые слова данных до добавления заполнения и коррекции ошибок.
     *
     * @return list<int>
     */
    public function codewords(string $data, bool $gs1 = false): array
    {
        $codewords = $this->encodeAscii($data);

        // Разделители GS внутри данных остаются ASCII-символом 29: так их возвращают сканеры.
        return $gs1 ? [self::FNC1, ...$codewords] : $codewords;
    }
        ...
        for ($row = 0; $row < $rows; $row++) {
            ...
            for ($col = 0; $col < $cols; $col++) {
                ...
                if ($colInRegion === $dataRegionCols + 1) {
                    // The first data row is dark; row zero belongs to the top border.
                    $matrix[$row][$col] = $rowInRegion % 2 === 1;
                    continue;
                }
